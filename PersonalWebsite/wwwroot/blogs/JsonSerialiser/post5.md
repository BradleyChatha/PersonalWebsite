...
To attach a UDA onto something, you use the form `@TypeName`, e.g:
```
...
### How to access a UDA on a symbol
...
line of the `static foreach` (which performs the actual serialisation)
behind a `static if` that checks whether the field is to be ignored or not:
...
```
It's a bit of an iffy way to go about things compared to simply being able to `continue`
...
As you can see, the JSON output for `PersonWithUDAs` now uses
...
To finish off with our last UDA, we need to employ `static if`
...
Just like with `@Name`, we're using the same testing code as before, but
...
* @MatchesRegex - Strings must match the provided regex. (See also <a href="https://dlang.org/phobos/std_regex.html">std.regex</a>)
