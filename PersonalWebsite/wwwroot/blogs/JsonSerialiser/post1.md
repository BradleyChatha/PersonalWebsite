...
This first post will cover how to create two template functions, that will use several features of D's meta programminmg 
to determine how to serialise/deserialise most of D's primitive types.
...
and return a std.json#JSONValue which is the main struct that std.json uses to represent JSON values.
...
This means all we have to do to serialise a primitive type, is to construct a std.json#JSONValue, 
...
Simple enough, but right now, we're letting the user pass through any type they want into our `serialise` function, 
...
classes, and enums, all of which can't be serialised directly by std.json#JSONValue:JSONValue's constructor.
...
Basically, static if is like a normal if statement, except: 
...
* Any code that isn't inside a static if's passing block (either the if or the else) doesn't get compiled into the program.

For example, say I wanted to have a compile time flag in my code that determined whether my program decides to do any logging or not, 
...
To start, we'll make a deserialise function that takes a JSONValue, a type parameter (T), and returns a T.
...
In the event that all of our static ifs fail to have their conditions met, we can then fall back to a 
...
So get an import std.conv going somewhere, and let's improve our deserialiser.
